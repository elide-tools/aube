...
pub fn host_triple() -> (&'static str, &'static str, &'static str) {
    ...
    // Detect libc at runtime, not compile time. `cfg!(target_env = "musl")`
    // is the toolchain that built the aube binary, not the host's libc: a
    // static-musl build shipped to glibc users reported libc=musl everywhere
    // and installed prebuilts node could not load. The shared probe reads
    // /proc/self/maps with a glibc-first loader-scan fallback; see
    // `aube_util::libc`, which aube-runtime uses for the same decision.
