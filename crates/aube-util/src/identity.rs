...
pub struct Embedder {
    ...
    /// see in output, and the command name driving help/usage/errors.
    ...
    /// references and in usage/error output. Standalone aube uses `["aube"]`;
    /// an embedded host can use something like `["elide", "aube", "--"]`
    /// while keeping [`name`](Self::name) as the stable slug.
    ///
...
/// The active tool's user-facing invocation prefix as one display string, e.g.
/// `"elide aube --"`. Interned on first use so it can be handed to
/// `#[usage(bin = ...)]`, which requires `&'static str`.
pub fn command_prefix_display() -> &'static str {
    static CACHE: OnceLock<String> = OnceLock::new();
    CACHE.get_or_init(|| command_prefix().join(" ")).as_str()
}
